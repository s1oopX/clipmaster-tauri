...
日期：2026-07-26（文档基线复核日）

当前公开版本线：`0.1.7`

0.1.3–0.1.7 主要补齐图片预览稳定性、旧路径兜底、主窗口快捷键、截图遮挡，以及 asset 协议 scope 修复与安全配置测试锁定，详见 `CHANGELOG.md`。
...
ClipMaster 继续保持本地优先、轻量、个人工具定位。后续不做：
...
截图能力只保留裁剪和简单标注：矩形、箭头、画笔、基础遮挡、撤销/重做，以及必要的重新框选。任何新功能都应优先减少打扰、减少常驻资源和减少用户设置负担。
...
- 截图体验收敛：只修补裁剪、基础标注和隐私遮挡的稳定性，不扩展复杂标注系统。
- 导入导出和备份恢复，仅在不引入账号或云服务的前提下考虑。
- Windows 代码签名。
