...
ClipMaster 是一款面向 Windows 桌面的本地剪贴板工具。它会在本机保存文本、链接、图片和截图历史，提供搜索、筛选、收藏、置顶、贴图、标注和清理能力，帮助用户找回临时复制内容并减少重复截图、重复复制和来回切换窗口的时间。

本项目不提供云同步、账号系统或远程遥测。数据默认保存在本机应用数据目录中，适合希望保留本地控制权的个人工作流、开发调试、文档编写、客服沟通和轻量图片复用场景。

## 当前状态

当前版本线为 `0.1.6`。项目已经覆盖日常可用的剪贴板历史、链接识别与打开、图片历史、区域截图、冻结屏幕选区、基础标注、桌面贴图、全局快捷键、系统托盘、设置、清理策略、数据迁移和 Windows 打包流程。

后续开发会保持轻量边界：截图只做裁剪和简单标注，不规划 OCR、滚动截图、云同步、自动更新、复杂团队/账号体系或富文本编辑器式标注系统。详情见 [Roadmap](./docs/ROADMAP.md)。
...
| 文件 | 适用场景 |
| --- | --- |
| `ClipMaster_0.1.6_x64-setup.exe` | 推荐安装包，适合大多数 Windows 用户 |
| `ClipMaster_0.1.6_x64_en-US.msi` | MSI 安装包，适合传统部署、企业环境或需要 MSI 的场景 |
| `SHA256SUMS.txt` | 发布文件校验信息 |

当前构建尚未进行代码签名，Windows SmartScreen 可能出现提示。请仅从本仓库 Release 页面下载，并根据 Release 附带的 SHA256 文件校验安装包。

本地发布产物目录说明见 [Release Artifacts](./docs/RELEASES.md)。

## 功能概览

| 模块 | 能力 |
| --- | --- |
| 剪贴板历史 | 自动记录文本、链接和图片剪贴板内容，支持复制回剪贴板 |
| 链接工作流 | 自动识别链接，按链接筛选，支持从系统默认浏览器打开 |
| 搜索与筛选 | 支持内容搜索、类型筛选、日期筛选、会话记录查看和加载更多 |
| 收藏与置顶 | 将重要记录固定在更容易找到的位置 |
| 图片工作流 | 保存原图、生成缩略图、预览图片、复制图片、钉到桌面 |
| 截图工作流 | 区域截图、冻结屏幕选区、自动复制、保存历史、重新框选、矩形/箭头/画笔/模糊/马赛克标注、撤销/重做 |
| 桌面贴图 | 将图片以置顶小窗方式贴到桌面，便于对照和复用 |
| 全局快捷键 | 支持快捷键呼出/隐藏主窗口、聚焦搜索框，以及启动区域截图 |
| 系统托盘 | 关闭主窗口后隐藏到托盘，支持恢复和退出；托盘不可用时保持主窗口可见 |
| 设置管理 | 支持主窗口快捷键、截图快捷键、截图延迟、保留数量、时区、语言、开机自启动等配置 |
| 数据清理 | 支持按数量、天数、图片生命周期清理，以及一键清空全部历史 |
| 数据迁移 | 内置旧数据目录迁移和数据库 schema migration |

## 产品边界

ClipMaster 会保持为本地优先的轻量工具。截图功能只聚焦裁剪、基础形状、箭头、画笔、隐私遮挡和撤销/重做；不会扩展成 OCR、滚动截图、云同步、自动更新、团队协作、账号系统或富文本编辑器式标注平台。
...
- 支持矩形、箭头、画笔、模糊和马赛克标注，支持撤销/重做，最终输出会包含标注内容。
- 支持重新框选和截图后直接钉到桌面。
- 成熟截图工具对照和后续取舍见 [Screenshot Feature Review](./docs/SCREENSHOT_REVIEW.md)。
